...
def get_directory_name(config: cfg.Configuration, additional: str = None, with_datetime: bool = False) -> str:
    ...
    suffix += ''.join(['_{}'.format(value) for value in [
        config.patch_size[-1],
        config.epochs, config.batch_size_training, str(config.learning_rate).replace('.', '-')
    ]])
